...
- **NVIDIA RTX hardware ray tracing (RT cores in GeForce 40-series, A100/H100)** — dedicated silicon executes ray-AABB and ray-triangle intersections billions of times per second to render Cyberpunk 2077, Minecraft RTX, and offline films with Pixar's RenderMan.
- **Oil and gas seismic ray tracing (Schlumberger Petrel, CGG GeoSoftware)** — petroleum geologists shoot virtual rays through subsurface velocity models to predict reflections at salt-dome boundaries, guiding multibillion-dollar drilling decisions.
- **Radiation therapy planning (Varian Eclipse, Elekta Monaco)** — treatment-planning systems trace photon and proton rays from a linear accelerator through CT scans of the patient to compute dose distributions that destroy tumors while sparing healthy tissue.
- **Autonomous driving simulators (NVIDIA DRIVE Sim, CARLA)** — synthetic LIDAR returns are generated by ray-casting against virtual world geometry, providing the millions of simulated km Tesla and Waymo need to validate self-driving stacks safely.
