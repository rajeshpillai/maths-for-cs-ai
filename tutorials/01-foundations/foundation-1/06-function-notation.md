...
- **Board Exam / JEE (CBSE Class 11, Chapter 2 — Relations and Functions)** — appears in NCERT Exercises 2.1 (Cartesian product), 2.2 (relations, domain/range), 2.3 (functions, special functions like signum, modulus, greatest-integer). High-weightage JEE question types: (i) finding domain/range of $f(x) = \sqrt{9 - x^2}$, $f(x) = 1/(x-7)$, or $f(x) = (x^2 + 3x + 5)/(x^2 - 5x + 4)$ — always factor the denominator first; (ii) "given $A \times A$ has 9 elements and $(-1, 0), (0, 1) \in A \times A$, find $A$" — recover $A$ from observed components plus the cardinality constraint.
- **Common pitfalls drilled in NCERT** — (a) $(a, b) \neq \{a, b\}$: order matters for *ordered pairs*, not sets; (b) $A \times B \neq B \times A$ in general (commutative only when $A = B$); (c) $\lfloor -2.5 \rfloor = -3$, **not** $-2$ — floor steps *left* on the number line; (d) substituting $x = 0$ into $|x|/x$ gives $0/0$ — the signum function defines $\text{sgn}(0) = 0$ separately; (e) the sum function $(f + g)$ is defined only on the *intersection* of domains, not the union.
