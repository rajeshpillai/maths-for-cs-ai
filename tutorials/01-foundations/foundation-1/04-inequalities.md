...
- **Board Exam / JEE (CBSE Class 11, Chapter 6 — Linear Inequalities)** — NCERT focuses on linear inequalities in one variable. Exercises 6.1 (one-variable, integer/natural/real solution sets), 6.2 (two-variable inequalities, half-plane graphing), 6.3 (systems of inequalities — feasible regions). High-weightage JEE Mains items: (i) compound inequalities like $-3 \le 4 - 7x/2 \le 18$ — manipulate all three parts simultaneously; (ii) solving fractional inequalities like $(2x - 1)/3 \ge (3x - 2)/4 - (2 - x)/5$ — combine over a common denominator first; (iii) representing the solution on a number line with open/closed circles. Class 12 carries this into **Linear Programming**.
- **Common pitfalls drilled in NCERT** — (a) **the flip rule**: multiplying or dividing both sides by a *negative* number flips $<$ to $>$ and vice-versa — this is the single most common mistake; (b) addition or subtraction *never* flips the sign; (c) "natural number" in NCERT excludes 0 and negatives — if the question says "$x \in \mathbb{N}$" and the interval is $(-\infty, 4)$, the answer is $\{1, 2, 3\}$, not $\{0, 1, 2, 3\}$; (d) infinity always gets a *round* bracket in interval notation — you cannot include $\infty$.
