...
- **Board Exam / JEE (CBSE Class 11, Chapter 10 — Straight Lines)** — NCERT introduces slope $m = \tan\theta$ (where $\theta$ is the **angle of inclination** with the positive x-axis, $0° \le \theta < 180°$), parallel ($m_1 = m_2$) and perpendicular ($m_1 m_2 = -1$) conditions, and the **five standard forms**: slope-intercept $y = mx + c$, point-slope $y - y_1 = m(x - x_1)$, two-point, intercept $x/a + y/b = 1$, and general $Ax + By + C = 0$. Exercises 10.1–10.3 drill: (i) finding the equation given a point + angle (e.g. $(2, 3)$ at $75°$ — slope $\tan 75°$ from compound-angle); (ii) collinearity by area-formula or equal-slope test; (iii) midpoint of axis-intersecting segment $\Rightarrow x/a + y/b = 2$; (iv) linear models of physical data (rod thermal expansion, demand vs price). JEE Mains adds **distance from a point to a line** $|Ax_0 + By_0 + C|/\sqrt{A^2 + B^2}$ and **angle between two lines** $\tan\phi = |(m_2 - m_1)/(1 + m_1 m_2)|$.
- **Common pitfalls drilled in NCERT** — (a) a *vertical line* has slope **undefined** (not zero, not infinity); (b) parallel ↔ slopes *equal*, perpendicular ↔ slopes *product = $-1$*, **not** $m_2 = -m_1$; (c) intercept form $x/a + y/b = 1$ requires both intercepts non-zero — use point-slope or $y = mx$ for lines through the origin; (d) "$2$ units to the left of the origin" means $x = -2$, not $x = 2$.
