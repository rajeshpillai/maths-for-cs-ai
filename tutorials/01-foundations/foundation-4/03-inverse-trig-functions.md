...
- **Board Exam / JEE (CBSE Class 12, Chapter 2 — Inverse Trigonometric Functions)** — NCERT defines the six **principal-value branches** that you must memorise: $\sin^{-1}: [-1,1] \to [-\pi/2, \pi/2]$; $\cos^{-1}: [-1,1] \to [0, \pi]$; $\tan^{-1}: \mathbb{R} \to (-\pi/2, \pi/2)$; $\cot^{-1}: \mathbb{R} \to (0, \pi)$; $\sec^{-1}, \csc^{-1}$ similar with the $|x| \ge 1$ domain. Exercises 2.1–2.2 drill: (i) computing principal values like $\sin^{-1}(-1/2) = -\pi/6$, $\cos^{-1}(-1/2) = 2\pi/3$ (note: not $-\pi/3$); (ii) the **complementary identities** $\sin^{-1}x + \cos^{-1}x = \pi/2$, $\tan^{-1}x + \cot^{-1}x = \pi/2$, $\sec^{-1}x + \csc^{-1}x = \pi/2$; (iii) the $\tan^{-1}$ sum formula $\tan^{-1}x + \tan^{-1}y = \tan^{-1}((x+y)/(1-xy))$ when $xy < 1$, with $\pm\pi$ correction otherwise; (iv) reducing out-of-branch compositions like $\sin^{-1}(\sin(2\pi/3))$ — *not* $2\pi/3$, but $\pi/3$.
- **Common pitfalls drilled in NCERT** — (a) $\sin^{-1}x \neq 1/\sin x$: the "$-1$" is the inverse, not a power; (b) $\sin^{-1}(\sin\theta) = \theta$ **only** when $\theta \in [-\pi/2, \pi/2]$ — outside that, reduce first; (c) negative-input rules differ: $\sin^{-1}(-x) = -\sin^{-1}x$ (odd) but $\cos^{-1}(-x) = \pi - \cos^{-1}x$; (d) the $2\tan^{-1}x = \sin^{-1}(2x/(1+x^2))$ identity holds only for $|x| \le 1$; outside, the right side lands in a different branch.
