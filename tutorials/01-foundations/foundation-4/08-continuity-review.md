...
- **Board Exam / JEE (CBSE Class 12, Chapter 5 — Continuity and Differentiability)** — NCERT Ex 5.1 drills the **three-part test** for continuity at $x = a$: (i) $f(a)$ defined, (ii) $\lim_{x \to a} f(x)$ exists (LHL = RHL), (iii) $\lim_{x \to a} f(x) = f(a)$. Standard JEE Mains Q-types: (i) classify discontinuities (removable / jump / infinite); (ii) the **find-the-constant** problem: a piecewise function $f$ is declared continuous at $a$, find unknown $\lambda, k$ by setting LHL = RHL = $f(a)$ — sometimes the equation is inconsistent (e.g. $0 = 1$), in which case **no value works** and you must say so; (iii) using the substitution $x = a - h$ for LHL and $x = a + h$ for RHL with $h \to 0^+$.
- **Common pitfalls drilled in NCERT** — (a) all three conditions must hold, not just two; (b) at piecewise boundary points, double-check which piece applies at $a - h$ versus $a + h$; (c) $\tan x, \sec x, \cot x, \csc x$ are **not** continuous on all of $\mathbb{R}$ — discontinuities at the singularities; (d) $1/x$ is continuous on its domain $\mathbb{R} \setminus \{0\}$, but it's *not* "continuous at $0$" because $0$ is not in the domain.
