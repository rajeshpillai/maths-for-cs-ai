...
- **Board Exam / JEE (CBSE Class 12, Chapter 11 — Three-Dimensional Geometry)** — NCERT introduces **direction cosines** $l = \cos\alpha, m = \cos\beta, n = \cos\gamma$ (with $l^2 + m^2 + n^2 = 1$) and **direction ratios** (any non-zero scalar multiples). The two equation forms for a 3-D line: **vector form** $\vec{r} = \vec{a} + \lambda\vec{b}$ (point + direction), **Cartesian form** $\frac{x - x_1}{a} = \frac{y - y_1}{b} = \frac{z - z_1}{c}$. Plane forms: vector $\vec{r} \cdot \hat{n} = d$, Cartesian $A x + B y + C z = D$, intercept $x/a + y/b + z/c = 1$. Exercises 11.1–11.3 drill: (i) angle between two lines $\cos\theta = (\vec{b_1} \cdot \vec{b_2})/(|\vec{b_1}||\vec{b_2}|)$ — perpendicular iff dot product is 0, parallel iff cross product is $\vec{0}$; (ii) **shortest distance between two skew lines**: $\big|((\vec{a_2} - \vec{a_1}) \cdot (\vec{b_1} \times \vec{b_2})) / |\vec{b_1} \times \vec{b_2}|\big|$; (iii) distance from a point $(x_0, y_0, z_0)$ to plane $A x + B y + C z = D$: $|A x_0 + B y_0 + C z_0 - D|/\sqrt{A^2 + B^2 + C^2}$.
- **Common pitfalls drilled in NCERT** — (a) the direction-cosine identity $l^2 + m^2 + n^2 = 1$ — useful as a check; the direction-ratio "identity" ($a^2 + b^2 + c^2 = ?$) does *not* equal 1 in general; (b) signs matter for direction cosines: a line and its reverse give opposite signs for $l, m, n$; (c) the **shortest distance** formula uses the **scalar triple product** in the numerator and the **magnitude of the cross product** in the denominator — easy to get them swapped under exam pressure.
