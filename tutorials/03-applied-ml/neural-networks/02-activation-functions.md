...
- **Industry / LLMs**: **GPT-4**, **Claude**, and **Gemini** use **GELU** in every transformer feed-forward block; **LLaMA-3** and **Mistral** switched to **SwiGLU** (a swish-gated variant) — every token generated by these models passes through millions of these activations.
- **Engineering / Image generation**: **Stable Diffusion XL** and **Flux.1** use **SiLU/Swish** throughout the U-Net denoiser; the smooth derivative is what makes diffusion training stable across thousands of denoising steps.
- **Business / Recommendation systems**: **YouTube's recommender** and **Netflix's ranking model** use ReLU in the deep tower and softmax at the output; softmax over millions of candidate videos is what produces the personalised feed every user sees.
- **Engineering / Speech**: **Whisper** (OpenAI) and **NVIDIA NeMo Parakeet** use GELU in their encoder; the output layer applies softmax over ~50,000 BPE tokens to transcribe in 90+ languages — the same softmax derivation in this lesson.
