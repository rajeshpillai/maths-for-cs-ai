...
- **Board Exam / JEE (CBSE Class 11, Chapter 7 — Permutations and Combinations)** — NCERT builds the chapter from the **Fundamental Principle of Counting** (FPC) → factorial → ${}^nP_r = n!/(n-r)!$ → ${}^nC_r = n!/(r!\,(n-r)!)$. Exercises 7.1–7.4 drill: (i) FPC: shirt-trouser, lock codes, road-route problems ("AND" multiplies, "OR" adds); (ii) words from `PERMUTATIONS` etc. with identical-letter division $n!/(p!\,q!\,r!)$; (iii) circular permutations $(n-1)!$, with division by 2 for necklaces; (iv) committee/team selection like "11 from 17 with exactly 4 bowlers from 5 available". JEE Mains adds **Pascal's identity** ${}^nC_r + {}^nC_{r-1} = {}^{n+1}C_r$ and the symmetry ${}^nC_r = {}^nC_{n-r}$.
- **Common pitfalls drilled in NCERT** — (a) confusing permutation with combination — the keyword test: *order matters → use ${}^nP_r$, order doesn't → use ${}^nC_r$*; (b) the convention $0! = 1$ (forced by recursion $1! = 1 \cdot 0!$); (c) for 3-digit numbers from $\{0, \ldots, 9\}$ with repetition allowed, the hundreds slot has only 9 choices (not 10) since leading 0 is excluded; (d) "OR" with mutually-exclusive cases adds, but if cases overlap you need inclusion-exclusion.
