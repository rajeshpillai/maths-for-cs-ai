...
- **Board Exam / JEE (CBSE Class 12, Chapter 5 — Continuity and Differentiability + Chapter 6 — Application of Derivatives)** — Ch 5 Ex 5.2–5.7 drill the **standard derivatives** ($\sin, \cos, \tan, e^x, \log x, x^n$, all six inverse trig), the **algebra of derivatives** (sum, product, quotient), the **chain rule**, and the specialised techniques: implicit differentiation, parametric form ($\frac{dy}{dx} = (dy/dt)/(dx/dt)$), **logarithmic differentiation** for products / quotients / variable-base powers like $y = x^x$ ($\Rightarrow \log y = x \log x \Rightarrow$ differentiate). Ch 6 (Application of Derivatives) Ex 6.1–6.5 add: (i) rate of change ($dV/dt$, $dA/dt$); (ii) increasing/decreasing intervals via sign of $f'(x)$; (iii) tangents and normals; (iv) **maxima/minima** via the **first-derivative test** and the **second-derivative test**; (v) **approximations** $f(a + h) \approx f(a) + h f'(a)$. JEE Mains adds Rolle's theorem and the Mean Value Theorem.
- **Common pitfalls drilled in NCERT** — (a) differentiable ⇒ continuous, but not vice versa: $|x|$ at $0$ is continuous but has LHD = $-1 \neq +1 = $ RHD, hence non-differentiable; (b) for $y = x^x$, you cannot use either $n x^{n-1}$ or $a^x \ln a$ directly — must take $\log$ first; (c) at endpoints of a closed interval, only the relevant one-sided derivative is required.
