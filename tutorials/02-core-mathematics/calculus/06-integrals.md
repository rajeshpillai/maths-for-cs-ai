...
- **Board Exam / JEE (CBSE Class 12, Chapter 7 — Integrals + Chapter 8 — Application of Integrals)** — Ch 7 introduces the **indefinite integral** as the antiderivative ($\int f(x)\,dx = F(x) + C$ where $F'(x) = f(x)$) and drills the major techniques: (i) **substitution**, (ii) **partial fractions** (e.g. $\int dx/((x-1)(x-2)) = \log|x-1| - \log|x-2|$), (iii) **integration by parts** $\int u\,dv = uv - \int v\,du$ (use the **ILATE** rule to choose $u$: Inverse-trig, Logarithmic, Algebraic, Trigonometric, Exponential), (iv) trig substitutions for $\sqrt{a^2 - x^2}, \sqrt{a^2 + x^2}, \sqrt{x^2 - a^2}$. Ch 8 (Application of Integrals) Ex 8.1–8.2 use **definite integrals** for areas: $A = \int_a^b f(x)\,dx$ between curve, x-axis, and $x = a, b$; $A = \int_a^b [f(x) - g(x)]\,dx$ between two curves; horizontal slicing $A = \int_c^d x(y)\,dy$ when easier. Standard JEE problems: area enclosed by a circle / ellipse / parabola.
- **Common pitfalls drilled in NCERT** — (a) when $f(x) < 0$ on $[a, b]$, $\int_a^b f(x)\,dx$ is negative — for area, take $|·|$ or split at zeros and use $\int |f|\,dx$; (b) sketch the region first, find intersections, then decide vertical or horizontal slicing — most marks are lost on the *sketch*, not the integration; (c) for $\int_0^{\pi/2} \sin^2 x\,dx$, never expand $\sin^2 x$ into "$\sin x \cdot \sin x$" — use $\sin^2 x = (1 - \cos 2x)/2$; (d) the **ILATE** order: pick $u$ as the higher item in the list to make $du$ simpler.
