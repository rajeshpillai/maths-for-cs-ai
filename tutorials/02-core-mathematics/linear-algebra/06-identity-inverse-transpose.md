...
- **Board Exam / JEE (CBSE Class 12, Chapter 3 — Matrices, plus Ch 4 Determinants)** — NCERT Ex 3.3–3.4 cover the **transpose** $(A^T)_{ij} = a_{ji}$ with the order-reversing identities $(AB)^T = B^T A^T$ and $(A^T)^{-1} = (A^{-1})^T$, plus the **symmetric / skew-symmetric** decomposition $A = \tfrac{1}{2}(A + A^T) + \tfrac{1}{2}(A - A^T)$. Inverse via elementary row operations is on the exam: write the augmented matrix $[A \mid I]$ and reduce until the left half is $I$; the right half is $A^{-1}$. JEE Mains Q-types: (i) for $A = \begin{pmatrix} 2 & 5 \\ 1 & 3 \end{pmatrix}$, find $A^{-1}$ by row ops; (ii) prove $(AB)^{-1} = B^{-1}A^{-1}$; (iii) classify a given $3 \times 3$ matrix as symmetric / skew / neither.
- **Common pitfalls drilled in NCERT** — (a) the inverse exists **only** for square matrices, and only when $\det A \neq 0$ (the determinant test from Ch 4); (b) order **reverses** in both transpose and inverse of a product; (c) skew-symmetric matrices have zero diagonal; (d) for $2 \times 2$ matrix $\begin{pmatrix} a & b \\ c & d \end{pmatrix}$, $A^{-1} = \tfrac{1}{ad-bc}\begin{pmatrix} d & -b \\ -c & a \end{pmatrix}$ — memorise this short formula.
