...
- **Board Exam / JEE (CBSE Class 12, Chapter 3 — Matrices, and Chapter 4 — Determinants)** — NCERT teaches **elementary row operations** ($R_i \leftrightarrow R_j$, $R_i \to k R_i$, $R_i \to R_i + k R_j$) as the engine for both finding $A^{-1}$ and solving the linear system $A X = B$. The matrix-method workflow: write $A X = B$, find $A^{-1}$, multiply $X = A^{-1} B$. Class 12 Ch 4 (Determinants) supplies the second method via Cramer's rule and the adjoint formula $A^{-1} = \tfrac{1}{\det A}\,\text{adj}(A)$. JEE Mains Q-types: (i) classify a system as having a unique / no / infinite solutions by checking $\det A$ and consistency; (ii) solve a $3 \times 3$ system by row reduction.
- **Common pitfalls drilled in NCERT** — (a) the matrix method only gives a **unique** solution when $\det A \neq 0$ (i.e. $A$ is invertible); when $\det A = 0$, the system is either inconsistent or has infinitely many solutions — check via the augmented matrix; (b) row operations preserve the solution set, but do *not* preserve the determinant unchanged (swapping rows flips its sign; multiplying a row by $k$ multiplies $\det$ by $k$).
